...
def GetCenterImage(raster):
    ...
    return [geo_ext[0][0], geo_ext[0][1]]
    ...
    return Template(
        u"""function (e) {
            if ({{m}}.flagControl._currentState.stateName == "undo-flag"){
                jQuery.get(\"https://europe-west1-rg-maps-216117.cloudfunctions.net/fat_lambda?method=flag?kid="""+key['kid']+u"""club="""+key['club']+u"""")
                {{m}}.flagControl.state("set-flag")
            }
        }""")


events = json.load(open(manifest_json, 'r'))

for (club, es) in events.items():
    for (kid, event) in es.items():
      if not event['worldfile']['valid']: continue
      key = club + "-" + kid
      print(key)
      image_vrt = vrt_dir + '/' + key + '.vrt'
